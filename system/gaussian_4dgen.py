...
from ..utils.arap_utils import KNNARAPCoach

from torchmetrics import PeakSignalNoiseRatio
import torchvision


@threestudio.register("gaussian-4dgen-system")
class Gaussian4DGen(BaseLift3DSystem):
    ...
    def _compute_arap_energy(self, tgt_timestamp=None, tgt_frame_idx=None):
        ...
        for i in range(vert_timed_xyz.shape[0]):
            loss_arap += self.arap_coach.compute_arap_energy(xyz_prime=vert_timed_xyz[i])
        return loss_arap
    ...
    def on_train_batch_start(self, batch, batch_idx, unused=0):
        ...
        if self.global_step == self.cfg.freq.milestone_arap_reg and self.arap_coach is None:
            self.arap_coach = KNNARAPCoach(
                self.geometry.get_xyz,
                self.geometry._deform_graph_connectivity,
                self.device 
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        out_zero123 = self.training_substep(
            batch, batch_idx, guidance="zero123", render_sparse_gs=False)
        total_loss += out_zero123["loss"]

        out_ref = self.training_substep(
            batch, batch_idx, guidance="ref", render_sparse_gs=False)
